...
class EventMgmtEngine {
    ...
    dispatchToSampleService(event, callback) {
        let queue = "smartpai-service";
        let allowedActions = ['create',];

        if (allowedActions.includes(event.targetAction)) {
            this.pushToQueue(queue, event, flag => callback(flag));
    ...
    pushToQueue(queue, payload, callback) {
